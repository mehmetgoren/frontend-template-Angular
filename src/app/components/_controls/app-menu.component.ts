...
export class AppMenuComponent implements OnInit {
    ...
    constructor(public app: DefaultComponent, private router: Router, private adminPanelService: AdminPanelService) {}

    ngOnInit() {
        let user = AppStorage.getUserIfNotAuthenticatedThenLogOut();
        this.model = [];
        this.adminPanelService.createMenu(user.RoleId).subscribe(data => {
            if (data && data.length) {
                data.forEach(menuItem => {
                    ...
                    if (menuItem.Childs && menuItem.Childs.length > 0) {
                        ...
                        menuItem.Childs.forEach(child => {
                            //item.items.push({ label: child.Name, icon: child.Image, routerLink:['/' + child.Route]});
                            item.items.push({
                                label: child.Name, icon: child.Image, route: child.Route, command: (event) => {//'route: child.Route' is added dynamically, thanks to javascript...
...
export class AppSubMenuComponent {
    ...
    constructor(public app: DefaultComponent) {}
